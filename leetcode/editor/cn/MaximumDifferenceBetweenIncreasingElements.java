package editor.cn;//给你一个下标从 0 开始的整数数组 nums ，该数组的大小为 n ，请你计算 nums[j] - nums[i] 能求得的 最大差值 ，其中 0 <=
...
//Java：|2016|增量元素之间的最大差值|MaximumDifferenceBetweenIncreasingElements|
...
class Solution {
    public int maximumDifference(int[] nums) {
        ...
        for (int i = 1; i < nums.length; ++i) {
            if(nums[i] > nums[min]) {
                ans = Math.max(ans, nums[i] - nums[min]);
            }else {
                min = i;
            }
        }
