package editor.cn;//一所学校里有一些班级，每个班级里有一些学生，现在每个班都会进行一场期末考试。给你一个二维数组 classes ，其中 classes[i] = [
...
//Java：|1792|最大平均通过率|MaximumAveragePassRatio|
public class MaximumAveragePassRatio{
    public static void main(String[] args) {
        Solution solution = new MaximumAveragePassRatio().new Solution();
        // TO TEST
        System.out.println(solution.maxAverageRatio(new int[][]{{1,2},{3,5},{2,2}},2));
    }
    ...
    public double maxAverageRatio(int[][] classes, int extraStudents) {
        PriorityQueue priorityQueue = new PriorityQueue(new Comparator() {
            ...
            public int compare(Object a, Object b) {
                ...
                return (aInt[0]+1.0)/(aInt[1]+1)-(aInt[0]+0.0)/(aInt[1])-(bInt[0]+1.0)/(bInt[1]+1)+(bInt[0]+0.0)/(bInt[1])>0?-1:1;
            }
        });
