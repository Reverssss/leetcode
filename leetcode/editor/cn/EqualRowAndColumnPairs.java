package editor.cn;//给你一个下标从 0 开始、大小为 n x n 的整数矩阵 grid ，返回满足 Ri 行和 Cj 列相等的行列对 (Ri, Cj) 的数目。
//
...
//Java：|2352|相等行列对|EqualRowAndColumnPairs|
...
class Solution {
    public int equalPairs(int[][] grid) {
        ...
        for(int i = 0 ; i < grid.length; i++){
            ...
            int hash = Arrays.hashCode(temp);
            res += hashtable.getOrDefault(hash,0);
        }
        return  res;
    }
}
