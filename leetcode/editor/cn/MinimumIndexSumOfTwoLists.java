package editor.cn;//假设 Andy 和 Doris 想在晚餐时选择一家餐厅，并且他们都有一个表示最喜爱餐厅的列表，每个餐厅的名字用字符串表示。
//
...
// 
// Related Topics 数组 哈希表 字符串 👍 146 👎 0

import java.util.ArrayList;
import java.util.HashMap;

//Java：|599|两个列表的最小索引总和|MinimumIndexSumOfTwoLists|
...
class Solution {
    public String[] findRestaurant(String[] list1, String[] list2) {
        ...
        for(int i = 0 ; i < list2.length ; i++){
            int num = map.getOrDefault(list2[i],-1);
            if(num != -1){
                if(min == num  + i){
                    ans.add(list2[i]);
                }else if(min > num + i){
