package editor.cn;//给你一个下标从 0 开始的整数数组 nums ，其长度是 2 的幂。
//
// 对 nums 执行下述算法： 
//
// 
    ...
    public int minMaxGame(int[] nums) {
        ...
        while (len!=1){
            ...
            for(int i = 0 ; i < nums.length/4 ; i++){
                nums[i*2] = Math.min(nums[i*4+0],nums[i*4+1]);
                nums[i*2+1] = Math.max(nums[i*4+2],nums[i*4+3]);
            }
        }
