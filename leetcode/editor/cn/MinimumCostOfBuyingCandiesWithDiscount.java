package editor.cn;//一家商店正在打折销售糖果。每购买 两个 糖果，商店会 免费 送一个糖果。
//
...
//Java：|2144|打折购买糖果的最小开销|MinimumCostOfBuyingCandiesWithDiscount|
...
class Solution {
    public int minimumCost(int[] cost) {
        ...
        cost =Arrays.stream(cost).sorted().toArray();
        int costLen = cost.length;
        int sum = 0;

        for(int i = 0 ; i < cost.length/3 ; i++){
            sum+=cost[costLen - 3*i-1];
            sum+=cost[costLen - 3*i-2];
        }
        if(cost.length%3 == 2){
            sum+=cost[0];
            sum+=cost[1];
