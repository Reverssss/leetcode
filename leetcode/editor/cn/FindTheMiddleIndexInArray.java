package editor.cn;//给你一个下标从 0 开始的整数数组 nums ，请你找到 最左边 的中间位置 middleIndex （也就是所有可能中间位置下标最小的一个）。
//
...
//Java：|1991|找到数组的中间位置|FindTheMiddleIndexInArray|
...
class Solution {
    public int findMiddleIndex(int[] nums) {
        int[] pivotSum = new int[nums.length];
        int sum = nums[0];
        ...
        for (int i = 0 ; i < nums.length ; i++){
            if(pivotSum[i] == sum-pivotSum[i]-nums[i])
                return i;
        }
        return -1;
    }
