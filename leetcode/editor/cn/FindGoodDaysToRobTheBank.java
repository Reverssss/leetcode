package editor.cn;//你和一群强盗准备打劫银行。给你一个下标从 0 开始的整数数组 security ，其中 security[i] 是第 i 天执勤警卫的数量。日子从 0 开始
//编号。同时给你一个整数 time 。 
//
...
// 
// Related Topics 数组 动态规划 前缀和 👍 10 👎 0

import java.util.ArrayList;
import java.util.List;

//Java：|2100|适合打劫银行的日子|FindGoodDaysToRobTheBank|
public class FindGoodDaysToRobTheBank{
    public static void main(String[] args) {
        ...
        System.out.println(solution.goodDaysToRobBank(new int[]{1,1,1,1,1},0));
        System.out.println(solution.goodDaysToRobBank(new int[]{1,2,3,4,5,6},2));
        System.out.println(solution.goodDaysToRobBank(new int[]{1},5));
    }
        ...
        public List<Integer> goodDaysToRobBank(int[] security, int time) {
            ...
            for (int i = 1; i < n; i++) {
                ...
                if (security[n - i - 1] <= security[n - i]) {
                    more[n - i - 1] = more[n - i] + 1;
                }
            }

            List<Integer> ans = new ArrayList<>();
            for (int i = time; i < n - time; i++) {
                if (less[i] >= time && more[i] >= time) {
                    ans.add(i);
                }
            }
            return ans;
