package editor.cn;//我们有 n 栋楼，编号从 0 到 n - 1 。每栋楼有若干员工。由于现在是换楼的季节，部分员工想要换一栋楼居住。
//
...
//Java：|1601|最多可达成的换楼请求数目|MaximumNumberOfAchievableTransferRequests|
public class MaximumNumberOfAchievableTransferRequests{
    public static void main(String[] args) {
        ...
        System.out.println(solution.maximumRequests(3,new int[][]{{2,0},{2,1},{0,1},{0,1}}));
    }
...
class Solution {
    public int maximumRequests(int n, int[][] requests) {
        ...
        int ans = 0;
        for(int i = 0 ; i < n ; i++){
            ans += Math.abs(buildings[i]);
        }

        return requests.length-ans/2;
    }
}
